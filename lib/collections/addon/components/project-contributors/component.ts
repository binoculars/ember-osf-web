...
export default class ProjectContributors extends Component.extend({
    ...
    querySubjects: task(function *(this: ProjectContributors, parents = 'null', tier = 0): IterableIterator<any> {
...
//     @notEmpty('currentSubjects') isValid!: boolean;
    ...
    @action
    deselect(this: ProjectContributors, index: number) {
    ...
    @action
    select(this: ProjectContributors, tier: number, selected: Taxonomy) {
